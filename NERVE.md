...
| 2026-04-16 | /api/clients/backfill | Backfill uitgebreid: scant nu ook client-projects (niet enkel sites) en maakt Client records aan voor klanten die alleen als Project bestaan (bv. Kristof/Luwaert/David). Sites worden gelinkt via clientId, projects blijven via naam-match totdat Project.clientId bestaat. | Claude |
| 2026-04-16 | /klanten | EditClientModal v2 cleanup: sectie "Contactgegevens & administratie" (phone/company/vatNumber/address/notes/paymentStatus) voor echte Client records, disclaimer en fee/uren velden conditioneel op siteCount>0, scrollbare modal. | Claude |
| 2026-04-16 | /klanten + /api/clients/[id] | **Bugfixes.** (1) Rename creëerde phantom klant: PATCH /api/clients/[id] syncd nu ook Projects (Project.clientName heeft geen FK naar Client) → oude-naam projects worden meerenamed. (2) Kan bestaande site aan klant koppelen: detail sheet krijgt Select-picker met alle unlinked sites; zet clientId/ownerType/clientName via PATCH /api/sites. Legacy klanten krijgen hint om eerst te backfillen. | Claude |
| 2026-04-16 | /klanten + cron | **Polish pass.** Site-picker smart matching: sites met matching clientName krijgen eigen 'Waarschijnlijke matches' sectie met ★-badge, andere sites gegroepeerd onder. Contract-expiry badges in klantenlijst (soon/urgent/expired) + hover toont einddatum. Cron check-alerts scant nu Client.contractEnd en maakt idempotente Alert bij <30d (high priority bij ≤7d of verlopen). Betere empty state voor klanten zonder sites (expliciete uitleg + project-count). paymentStatus 'paid/pending/cancelled' krijgen kleuren. | Claude |
